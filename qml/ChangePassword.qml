        ...
        Button {
            ...
            onClicked: {
                ...
                if (passwordEditText.text === confirmPasswordEditText.text) {
                    //两次输入相同
                    if (videoSocialControl.modifyPassword(mainPage.personalPage.netizen["id"], accountEditText.text, passwordEditText.text) === true) {
                        changePassword.visible = false
                    } else {
                        //如果不匹配，显示提示信息
                        errorText.text = "旧密码错误！"
                        errorText.visible = true
                        passwordTimer.running = true
                    }
                } else {
                    errorText.text = "两次密码不匹配！"
                    errorText.visible = true
                    passwordTimer.running = true
                }
